...
firefox_html5_detection=0
chromium_flash_detection=1
chromium_html5_detection=1
qutebrowser_flash_detection=1
...
isAppRunning()
{
    ...
    # Check if user want to detect Flash fullscreen on qutebrowser.
    if [ $qutebrowser_flash_detection == 1 ]; then
        if [[ "$activ_win_title" = *exe* || "$activ_win_title" = *utebrowser* ]]; then
            # Check if qutebrowser Flash process is running.
            qutebrowser_process=$(pgrep -lfc ".*QtWebEngineProcess.*")
            if [[ $qutebrowser_process -ge 1 ]]; then
                log "isAppRunning(): qutebrowser flash fullscreen detected"
                return 1
            fi
        fi
    fi
    ...
    if [ $chromium_pepper_flash_detection == 1 ]; then
        if [[ "$activ_win_title" = *utebrowser* ]]; then
            # Check if qutebrowser pepper Flash process is running.
            qutebrowser_flash_process=$(pgrep -lfc "QtWebEngineProcess --type=renderer ")
            if [[ $qutebrowser_flash_process -ge 1 ]]; then
                log "isAppRunning(): qutebrowser pepper flash fullscreen detected"
                return 1
            fi
        fi
    fi

    # Check if user want to detect HTML5 fullscreen on qutebrowser.
    if [ $qutebrowser_html5_detection == 1 ]; then
        if [[ "$activ_win_title" == *utebrowser* ]]; then
            # Check if qutebrowser process is running.
            if [[ $(pgrep -c "qutebrowser") -ge 1 ]]; then
                log "isAppRunning(): qutebrowser html5 fullscreen detected"
                return 1
            fi
        fi
    fi
